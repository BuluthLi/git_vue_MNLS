//使用过程出现多次load的情况，建议使用该项目中的vue-progressive-image插件
//可以通过这个插件（progressive-image）对源码分析，其实是使用inview()、添加不同的类名和img.oload更换图片的src
...
if (window.addEventListener && window.requestAnimationFrame && document.getElementsByClassName) window.addEventListener('load', function () {
    ...
    // image in view?
    //使用p进行计数，pItem是dom数组形式,但实际不是数组，使用pItem[p]的方式访问
    function inView() {
